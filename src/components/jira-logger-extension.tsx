...
/**
 * @file jira-logger-extension.tsx
 * @description Chrome extension version of the JiraLogger component
 * Modified to work without Next.js API routes, making direct API calls to Jira
 */

import { useState, useRef, useMemo, useEffect, useCallback } from "react";
import { useToast } from "@/hooks/use-toast";
import { formatMinutesToTime, parseWorklog, timeStringToMinutes, regenerateWorklogText } from "@/lib/parser";
import { logWorklogToJira, fetchUserWorklogs, type JiraCredentials } from "@/lib/jira-api";
...
export function JiraLoggerExtension() {
  ...
  useEffect(() => {
    chrome.storage.local.get(['jiraSettings'], (result) => {
      if (result.jiraSettings) {
        ...
        if (settings.url) settingsForm.setValue('url', settings.url);
        if (settings.email) settingsForm.setValue('email', settings.email);
        if (settings.apiToken) settingsForm.setValue('apiToken', settings.apiToken);
      }
    });
  }, []);

  // Auto-save settings to chrome.storage whenever they change
  useEffect(() => {
    const subscription: any = settingsForm.watch((values) => {
      chrome.storage.local.set({ jiraSettings: values });
    });
  ...
  const handleParse = async (textToParse: string) => {
    ...
    try {
      ...
      // Try to detect already-logged work
      let credentials: JiraCredentials | null = null;
      try {
        const result = await new Promise<any>((resolve) => {
          chrome.storage.local.get(['jiraSettings'], resolve);
        });
        if (result.jiraSettings) {
          credentials = result.jiraSettings;
        }
      } catch {}
      ...
      if (credentials && credentials.url && credentials.email && credentials.apiToken && dateFrom && dateTo) {
        try {
          const fetched = await fetchUserWorklogs(credentials, dateFrom, dateTo);
    ...
    if (!isValid) {
      ...
      const promises = entriesToLog.map(async (entry) => {
        ...
        try {
          ...
          await logWorklogToJira(credentials, entry.ticket, body);
            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent className="px-6 pt-2">
                  <p className="text-sm text-muted-foreground mb-4">
                      Enter your Jira credentials. They are stored locally in the extension.
                  </p>
                  <Form {...settingsForm}>
